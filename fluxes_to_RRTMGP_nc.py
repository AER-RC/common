...
class swRRTMGP():
  def __init__(self, config, reverseVertical=False):
    ...
    if self.specs != '': utils.file_check(self.specs)
  ...
  def initializeNC(self):
    ...
    if self.specs != '':
      ...
      if self.sw:
        ...
        # also need total solar irradiance
        inVarTSI = inObj.variables['total_solar_irradiance']
        tsiObj = specsObj.variables['total_solar_irradiance']
        tsi = np.array(tsiObj)
        tsiUnits = tsiObj.units
...
class lwRRTMGP(swRRTMGP):
  def __init__(self, config, reverseVertical=True):
    """
