  ...
  def computeBands(self, broadband=False, reverseVertical=False):
    ...
    if broadband:
      for iKey, key in enumerate(keys):
        ...
        if reverseVertical: fluxDict[key] = fluxDict[key][::-1, :]
      # end key loop
    ...
    if self.sw:
      ...
      if broadband:
        ...
        hr = RC.fluxToHR(diff)
        outVarBB[:] = hr[::-1, :] if reverseVertical else hr
        outObj.close()
      else:
        ...
        hr = RC.fluxToHR(diff)
        outVar[:] = hr[::-1, :, :] if reverseVertical else hr
      # endif BB
    # endif SW
  # end computeBands()
# end swRRTMGP()
...
if __name__ == '__main__':
  ...
  if args.longwave:
    ncRRTMGP = lwRRTMGP(ini)
    ncRRTMGP.initializeNC()
    ncRRTMGP.combineArr(test=args.lw_test)
    ncRRTMGP.computeBands(reverseVertical=True)
    ncRRTMGP.computeBands(reverseVertical=True, broadband=True)
  else:
